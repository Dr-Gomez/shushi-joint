import { isAlpha, isArithmeticOperator, isAssignmentOperator, isBinaryOperator, isBoolKeyword, isDigit, isIoKeyword, isLogicalKeyword, isLogicalOperator, isOperator, isSelectionKeyword, isSequencingKeyword, isString, isUnaryOperator, isWhitespace } from "./detection.ts";
...
function handleError(input: string, index: number): TokenWrapper {
    ...
    while (!isDigit(input[index]) && !isAlpha(input[index]) && !isOperator(input[index]) && !isString(input[index]) && index < input.length) {
        index++
    }

    const error = `"${input.substring(startIndex, index)}" is not recognized by the lexer as a construct`
