...
export function yardTokens(tokens: Token[]){
    ...
    while (tokenIndex < tokens.length) {
        ...
        if (token.type === TokenType.INT_NUM || token.type === TokenType.REAL_NUM || token.type === TokenType.IDENTIFIER || token.type === TokenType.STRING) {
        ...
        } else if (token.type === TokenType.LEFT_ENCAPSULATOR && token.value === "(") {
            operators.push(token);
        } else if (token.type === TokenType.RIGHT_ENCAPSULATOR && token.value === ")") {
            while (
                operators.length > 0 &&
            ...
            while (operators.length > 0) {
                ...
                if (op.type === TokenType.LEFT_ENCAPSULATOR || op.type === TokenType.RIGHT_ENCAPSULATOR) {
                    console.error("MISMATCHED PARENTHESES");
                }
                output.push(op);
            }
    ...
    while (operators.length > 0) {
        const op = operators.pop()!;
        if ((op.type === TokenType.LEFT_ENCAPSULATOR && op.value === ")") || (op.type === TokenType.RIGHT_ENCAPSULATOR && op.value === ")")) {
            console.error("MISMATCHED PARENTHESES");
        }
        output.push(op);
    }
